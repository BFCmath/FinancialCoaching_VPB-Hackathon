...
class TransactionHistoryFetcher:
    ...
    def _execute_tool(self, tool_name: str, tool_args: Dict) -> Any:
        ...
        try:
            # Execute the tool using invoke() method like jar_test
...
def quick_transaction_lookup(jar_name: str = None, time_period: str = None, limit: int = 10) -> Dict[str, Any]:
    ...
    if time_period:
        return get_time_period_transactions.invoke({"jar_name": jar_name, "start_date": time_period, "limit": limit})
    else:
        return get_jar_transactions.invoke({"jar_name": jar_name, "limit": limit})
